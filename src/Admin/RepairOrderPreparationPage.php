...
/**
 * Add "Repair Items Management" page to the admin menu.
 */
add_action( 'admin_menu', 'workrequest_add_repair_order_preparation_page' );
function workrequest_add_repair_order_preparation_page() {
    add_submenu_page(
        ...
        'manage_options', // Capability required to access (e.g., 'manage_woocommerce' or 'edit_posts')
        'workrequest-repair-order-preparation', // Menu slug
        'workrequest_display_repair_order_preparation_page' // Callback function to display page content
    );
}
 ...
 */
function workrequest_display_repair_order_preparation_page() {
    ?>
    <div class="wrap">
        ...
        $args = array(
            ...
            'meta_query'     => array(
                // Only show repair requests that have selected products AND haven't been converted to an order yet
                array(
                    'key'     => '_workrequest_selected_products',
                    'value'   => serialize( '' ), // Find posts where this meta exists and is not an empty array
                    'compare' => '!=',
                    'type'    => 'BINARY' // Important for serialized data comparison
                ),
                array(
                    'key'     => '_workrequest_selected_products',
 ...
 * Moved from admin-repair-request.php to handle order creation logic centrally here.
 */
add_action( 'wp_ajax_workrequest_create_order', 'workrequest_create_order_ajax_handler' );
function workrequest_create_order_ajax_handler() {
    check_ajax_referer( 'workrequest_create_order_nonce', 'security' ); // Check nonce from JS localize

    if ( ! current_user_can( 'edit_posts' ) ) { // Or a more specific capability like 'manage_woocommerce'
        wp_send_json_error( array( 'message' => __( 'You do not have permission to create orders.', 'workrequest' ) ) );
    }

    $request_id = isset( $_POST['request_id'] ) ? absint( $_POST['request_id'] ) : 0;
    // product_data is expected as an array of objects/arrays:
    // [ { product_id: 123, quantity: 1, note: "..." }, { product_id: 456, quantity: 2, note: "..." } ]
    ...
    try {
        ...
        if ( $customer_user_id > 0 ) {
            ...
            // $order->set_address( array( 'first_name' => $customer->get_billing_first_name() ), 'billing' );
            // ...
        }
